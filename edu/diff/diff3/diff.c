...
void version(void) {
  ...
  printf("Written by William McCarthy, Tony Stark, and Eric Minh Hanh Nguyen\n\n");
}
...
int count1 = 0, count2 = 0, different = 0, both_sides = 0, left_only = 0;
...
void init_options_files(int argc, const char* argv[]) {
  ...
  if (showsidebyside) { both_sides = 1; }

  if (showleftcolumn) { left_only = 1; }
...
//  showoptions(files[0], files[1]);
  ...
  else if (report_identical && different && !showbrief) { exit(0); }
  ...
  while (p != NULL) {
    ...
    if (foundmatch) {
      while ((foundmatch = para_equal(p, q)) == 0) {
        para_print(q, both_sides ? printright : printrightside);
        q = para_next(q);
        qlast = q;
      }
      int is_equal = 0;
      for (int i = p->start, j = q->start; i != p->stop && j != q->stop; ++i, ++j) {
        if (strcmp(p->base[i],q->base[j]) != 0) {
          line_print(p, q, i, j, both_sides, left_only, suppresscommon);
          is_equal = 1;
        }
      }
      if (both_sides && !is_equal && !left_only && !suppresscommon) para_print(q, printboth);
      if (left_only && !is_equal) para_print(q, print_left_column);
      p = para_next(p);
      q = para_next(q);
    } else {
      para_print(p, both_sides ? printleftside : printleft);
      p = para_next(p);
    }
  }
  while (q != NULL) {
